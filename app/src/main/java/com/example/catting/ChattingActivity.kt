...
*/
        ...
        setChattingLog()
        chattingAdapter = ChattingAdapter(chattingLogList, catProfile, this)

        with(binding){
            title.text = catProfile.cName
            chattingRecycler.layoutManager = LinearLayoutManager(
                this@ChattingActivity,
                LinearLayoutManager.VERTICAL,
                false
            )
            chattingRecycler.adapter = chattingAdapter
            chattingRecycler.addOnScrollListener(object: RecyclerView.OnScrollListener(){
                override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
                    super.onScrolled(recyclerView, dx, dy)
                    // 스크롤이 최상단에 도달했는지 확인
                    if (!chattingRecycler.canScrollVertically(-1)) {
                        Log.d("ChattingActivity","최상단")
                        if(offset > 0) getChattingLog()
                    } else if(!chattingRecycler.canScrollVertically(1)) {
                        scrollButton.visibility = View.INVISIBLE
                    } else {
                        if(scrollButton.isInvisible) scrollButton.visibility = View.VISIBLE
                    }
                }
            })
            scrollButton.setOnClickListener{
                chattingRecycler.scrollToPosition(chattingAdapter.itemCount - 1)
            }
...
*/
                ...
                api.sendMessage(userChattingLog).enqueue(object: Callback<ChattingLog>{
                    override fun onResponse(
                        ...
                        val catChattingLog = response.body()!!
                        Thread {
                            runOnUiThread(Runnable {
                                kotlin.run {
                                    insertChattingLog(catChattingLog)
    ...
    fun setChattingLog(){
        CoroutineScope(Dispatchers.IO).launch {
            offset = logHelper.chattingLogDao().getMax()
            Log.d("ChattingActivity","offset : $offset")
            val chattingLog = logHelper.chattingLogDao()
                .getPart(
                    userInfo.uid,
                    catProfile.cid,
                    offset,
                    offset - LOADING_SIZE + 1
                )
            offset -= LOADING_SIZE
            withContext(Dispatchers.Main){
                chattingAdapter.addAllItem(0, chattingLog as ArrayList<ChattingLog>)
                binding.chattingRecycler.scrollToPosition(chattingAdapter.itemCount - 1)
            }
        }
    }
    ...
    fun getChattingLog(){
        CoroutineScope(Dispatchers.IO).launch {
            Log.d("ChattingActivity","${offset - LOADING_SIZE + 1}, $offset")
            val chattingLog = logHelper.chattingLogDao()
                .getPart(
                    userInfo.uid,
                    catProfile.cid,
            ...
            withContext(Dispatchers.Main){
                chattingAdapter.addAllItem(0, chattingLog as ArrayList<ChattingLog>)
                binding.chattingRecycler.scrollToPosition(LOADING_SIZE)
            }
        }
    }
    ...
    fun insertChattingLog(chattingLog: ChattingLog){
        CoroutineScope(Dispatchers.IO).launch {
            ...
            withContext(Dispatchers.Main){
                chattingAdapter.addItem(chattingLog)
                binding.chattingRecycler.scrollToPosition(chattingAdapter.itemCount - 1)
            }
        }
    }
...
class ChattingAdapter(val items:ArrayList<ChattingLog>, val catProfile: CatProfile, val chattingActivity: ChattingActivity) : RecyclerView.Adapter<RecyclerView.ViewHolder>(){
    ...
    inner class UserHolder(val binding: ItemChattingUserBubbleBinding): RecyclerView.ViewHolder(binding.root){
        fun setUserMessageLog(chattingLog: ChattingLog, position: Int) {
            binding.message.text = chattingLog.userMessage
            binding.log.setOnLongClickListener {
                val dlg = DeleteDialog(chattingActivity, "Are you sure to delete this log?")
                dlg.setOnOKClickedListener {
                    when(it){
                        "yes"->{
                            CoroutineScope(Dispatchers.IO).launch {
                                chattingActivity.logHelper.chattingLogDao().delete(chattingLog)
                                withContext(Dispatchers.Main){
                                    removeItem(position)
                                }
                            }
    ...
    inner class CatHolder(val binding: ItemChattingCatBubbleBinding): RecyclerView.ViewHolder(binding.root){
        fun setCatMessageLog(chattingLog: ChattingLog, position: Int){
            with(binding) {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if(holder is UserHolder){
            holder.setUserMessageLog(items[position], position)
        }
        else if(holder is CatHolder){
            holder.setCatMessageLog(items[position], position)
        }
    }
    ...
    fun addAllItem(index:Int ,chattingLogList: ArrayList<ChattingLog>){
        items.addAll(index, chattingLogList)
        notifyDataSetChanged()
    }
